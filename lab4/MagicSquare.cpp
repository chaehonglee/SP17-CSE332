...
int MagicSquare::turn()
{
	...
	int prompt_output = prompt(numPiece);
...
void MagicSquare::setup_board()
{
	...
	//erases the pieces already on the board from the saved game
	std::set<int>::iterator it = GameClass::msExisting.begin();
	for (it; it != msExisting.end(); ++it) {
		numPieces.erase(*it);
	}
}
...
ostream & operator<<(ostream & out, const MagicSquare & rGame)
{
	...
	for (unsigned int i = 0;i < dim*dim; ++i) {
		unsigned int current = unsigned int(rGame.mVector_Game_Board[i].piece_symbol_.length());
	...
	for (int ver = int(dim - 1); ver >= 0; --ver)
	{
		...
		for (int hor = 0; hor < int(dim); ++hor)
		{
			...
			int str_length = int(rGame.mVector_Game_Board[dim * ver + hor].piece_symbol_.length());
